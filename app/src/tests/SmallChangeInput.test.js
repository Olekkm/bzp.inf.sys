...
describe("Проверка S блока на малое изменение входа", () => {
    ...
    test("Проверка S блока на малое изменение входа", () => {
        let totalTests = 0;
        let fullDiff = 0;
        let partialDiff = 0;
        let failedDiff = 0;
        ...
        for (const block of blocks) {
            ...
            const lastCharIndex = abc.getKey(block[3]);
            const modifiedChar = RUS_CHARS[(lastCharIndex + 1) % 32];
            const blockModified = block.slice(0, 3) + modifiedChar;

            for (const key of keys) {
                const encoded1 = coder.Encode(block, key);
                const encoded2 = coder.Encode(blockModified, key);
                ...
                if (diffCount === 4) {
                    ...
                    failedDiff++;
                    failedCases.push({ block, blockModified, key, diffCount });
                }

                if (decoded1 !== block || decoded2 !== blockModified) {
                    failedCases.push({ block, blockModified, key, decoded1, decoded2 });
                }
            }
        }
